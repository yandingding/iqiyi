...
; (function () {
    class dataSplicing {
        constructor(){
            this.ulproductlist=$('tchoiceness-productList');
        }
        ...
        init(){
           ...
           $.ajax({
               ...
               url: "http://10.31.164.180/html/project/aiqiyi/php/getpicurl.php",
               dataType: "json",
               success: function (data) {
                //    console.log(data);
                   $.each(data,function(index,value){
                    //    console.log(value.url);
                      $('.tchoiceness-productPic  img').eq(index).attr({
                          'src':value.url
                      });
                      $('.tchoiceness-productTitle a').eq(index).html(value.title);
                      $('.tchoiceness-productSubTitle').eq(index).html(value.introd);
                      $('.tchoiceness-productPrice').eq(index).html(value.price);
                      $('.tchoiceness-productSale').eq(index).html(value.sales);
                      $('.tchoiceness-productPic').eq(index).attr({href:"details.html?sid="+value.sid})

                   });
                   
                   
                   $('.tchoiceness-productList img').lazyload({
                        effect: "fadeIn"
                   })
               }
           });
